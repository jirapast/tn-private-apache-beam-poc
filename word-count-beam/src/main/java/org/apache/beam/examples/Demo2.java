...
// https://github.com/apache/beam/blob/master/examples/java/src/main/java/org/apache/beam/examples/complete/game/LeaderBoard.java
    ...
    // PCollection<KV<Integer, Instant>> generatedIntegers = x.apply(
    PCollection<String> y = pipeline.apply(Create.of("")).apply(
        // ParDo.of(new DoFn<Void, KV<Integer, Instant>>() {
        ParDo.of(new DoFn<String, String>() {
            private int counter = 0;
            @ProcessElement
            // public void processElement(@Element Void element, OutputReceiver<KV<Integer, Instant>> out) {
            public void processElement(@Element String element, OutputReceiver<String> out)  throws InterruptedException {
                // Instant timestamp = Instant.now();
                // out.output(KV.of(0, timestamp));
                System.out.println("Processing: " + "---------------------");  // Print to console
                out.output("sdfsdafsdafsadfasdfsdf");
                out.output("43543543534534534534");
                out.output("11111111111Vyp");
                out.output("2222222222222");
                out.output("333333333333");
                out.output("5555555555");
                while (counter < 4) {
